...
interface Props {
  ...
  isOwner?: boolean
  onClose: () => void
  onUpdate?: (updated: Partial<Hunter>) => void
}
...
export default function HunterProfileOverlay({ hunter, isOwner = false, onClose, onUpdate }: Props) {
  const { login, token } = useAuth()
  const [isEditing, setIsEditing] = useState(false)
  const [form, setForm] = useState({ gender: hunter.gender, age: hunter.age, class: hunter.class })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await api.patch(`/api/hunters/${hunter.id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" }
      })

      const updatedHunter = res.data
      const newAvatarUrl = updatedHunter.user?.avatarUrl ?? avatarPreview
      ...
      const updated = {
        ...
        hunter: { ...savedUser.hunter, ...form }
      }
      localStorage.setItem("user", JSON.stringify(updated))
      if (token) login(token, updated)

      onUpdate?.({ ...form, avatarUrl: newAvatarUrl })
  ...
  const handleCancel = () => {
    setIsEditing(false)
    setAvatarPreview(hunter.avatarUrl)
    setSelectedFile(null)
    setError("")
  }
    ...
    <div className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#0f1115] border border-[#2d3748] rounded-xl w-full max-w-sm p-6 relative" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-4 mb-6">
          <div className="w-16 h-16 rounded-full bg-[#2d3748] border-2 border-[#39ff14] overflow-hidden flex items-center justify-center flex-shrink-0">
            {avatarPreview ? (
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <p className="text-[#e5e7eb] font-bold text-lg" style={{ fontFamily: "Orbitron, monospace" }}>{hunter.name}</p>
            ...
            <p className="text-[#9ca3af] text-xs" style={{ fontFamily: "Fira Code, monospace" }}>RANK SCORE: {hunter.rankScore}</p>
          </div>
        </div>
        ...
        {!isEditing ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-3">
              <div className="flex flex-col gap-1 flex-1">
                ...
                <select name="gender" value={form.gender} onChange={handleChange}
                  className="bg-[#050505] border border-[#2d3748] rounded px-3 py-2 text-[#e5e7eb] text-sm outline-none focus:border-[#39ff14]"
            ...
            {error && <p className="text-[#b7410e] text-xs" style={{ fontFamily: "Fira Code, monospace" }}>{error}</p>}
